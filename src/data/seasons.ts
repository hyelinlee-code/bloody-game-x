...
export const seasons: SeasonMeta[] = [
  ...
  },
  {
    ...
    format:
      '캐스팅 철학이 완전히 바뀐 시즌이다. 일반인 중심이던 초기와 달리 각종 두뇌 서바이벌에서 이름을 알린 참가자들을 모아 올스타 구도를 만들었고, 그만큼 서로의 플레이 스타일을 이미 알고 들어온다는 전제가 게임 자체를 바꿨다. 개인이 확보한 자금이 최종 우승 상금에 직접 영향을 주지 않도록 규칙이 조정돼, 돈을 모으는 것과 이기는 것이 분리됐다. 거점 이름도 바뀌어, 조건이 보장되는 위쪽은 낙원, 밀려난 쪽은 잔해로 불렸다. 잔해에는 참가자가 아닌 진행 인력이 상주하는 유령 카지노가 딸려 있어, 밀려난 쪽도 계속 판에 묶여 있었다.',
    winnerId: undefined,
    winnerNameKo: '장동민',
...
export const franchise = {
  ...
  reception:
    'Wavve 오리지널로 자리 잡으며 시즌을 거듭할수록 화제성이 커졌고, 특히 배신과 연합이 드러나는 회차마다 커뮤니티 반응이 집중되는 패턴을 보였다.',
};
